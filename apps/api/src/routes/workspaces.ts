...
export function registerWorkspaceRoutes(app: FastifyInstance, prisma: PrismaClient) {
  ...
  app.post('/workspaces', async (request, reply) => {
    ...
    await safeRecordStrategicDecisionEvent(prisma, {
      ...
      payload: { type: workspace.type, mode: workspace.mode, color: workspace.color }
    });
  ...
  app.patch('/workspaces/:workspaceId', async (request) => {
    ...
    const current = await prisma.workspace.findUniqueOrThrow({
      where: { id: params.workspaceId, clerkUserId },
      select: { id: true, name: true, mode: true, type: true }
    });
    ...
    if (current.mode !== updated.mode) {
      if (updated.mode === 'expansao') { impact += 3; notes.push('Frente promovida para expansão.'); }
      else if (updated.mode === 'standby') { impact -= 2; notes.push('Frente movida para standby.'); }
      else { impact += 1; notes.push('Frente ajustada para manutenção.'); }
    }
    ...
    if (notes.length > 0) {
      await safeRecordStrategicDecisionEvent(prisma, {
        ...
        payload: { previousMode: current.mode, nextMode: updated.mode, previousType: current.type, nextType: updated.type }
      });
    }
  ...
  app.delete('/workspaces/:workspaceId', async (request) => {
    ...
    if (!workspace) throw new Error('Frente não encontrada.');
    if (workspace.type === 'geral') throw new Error('Frente geral não pode ser excluída.');

    const [projectsCount, tasksCount] = await Promise.all([
      prisma.project.count({ where: { workspaceId: workspace.id, archivedAt: null } }),
      prisma.task.count({ where: { workspaceId: workspace.id, archivedAt: null } })
    ]);

    if (!query.force && (projectsCount > 0 || tasksCount > 0)) {
      throw new Error(`Frente possui ${projectsCount} projeto(s) e ${tasksCount} tarefa(s). Confirme exclusão forçada para continuar.`);
    }
    ...
    await safeRecordStrategicDecisionEvent(prisma, {
      ...
      payload: { deletedWorkspaceId: workspace.id, projectsCount, tasksCount }
    });

    return { ok: true, projectsCount, tasksCount };
  });
}
